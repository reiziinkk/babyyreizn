...
const vcard = 'BEGIN:VCARD\n' // KASIH WM GUE
            ...
            + 'FN:REIZIN ID\n' // JAN NGEYEL AJG
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=5521976394573:+55 219-7639-4573\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []   
limitawal = 20
...
*/
const ownerNumber = ["5521976394573@s.whatsapp.net","5521976394573@s.whatsapp.net"]
const botName = 'REIZIN BOTZ'
const ownerName = 'Reizin 𝗜𝗗'
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `[ *BEM-VINDO AO  ${mdata.subject}* ] \n___________________________\n@${num.split('@')[0]} JA VAI MANDANDO OS ADMS`
				...
				teks = `Mais um fdp saiu... @${num.split('@')[0]}👋* \n_nao mamou os adms direito_`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
*/
				...
				client.sendMessage(from, buff, image, {quoted: mek, caption: 'Melhor escrever você mesmo, man :*'})
				await limitAdd(sender)
				break
				case 'nulis2':
				case 'tulis2':
...
// NINJALOGO				
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}ninjalogo Ramlan & Gans`)
				reply(ind.wait())
...
//GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply(`[❗] exemplo : ${prefix}gemboktext 11 01 2021 & Ramlan e Nadia`)
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`[❗] exemplo : ${prefix}glitchtext Ramlan & Gans`)
					reply(ind.wait())
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply(`O texto está onde um? Exemplo : ${prefix}simi eusougay`)
					teks = body.slice(5)
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return client.sendMessage(from, 'Qual é o código do idioma | exemplo : ${prefix}tts pt ah yamate kudasai', text, {quoted: mek})
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (!isQuotedSticker) return reply('Responda ou marque o adesivo que você deseja fazer uma foto de qualquel desgraça >_<')
					reply(ind.wait())
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'aqui man [(^.^)]'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
